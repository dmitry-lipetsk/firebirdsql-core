...
IAttachment* RProvider::attach(CheckStatusWrapper*  status,
                               const char*          filename,
...
IAttachment* RProvider::attachDatabase(CheckStatusWrapper*  status,
                                       const char*          filename,
                                       unsigned int         dpb_length,
                                       const unsigned char* dpb)
{
									        ...
									        int                  const protocol)
{
	...
	if (protocol >= PROTOCOL_VERSION13)
	{
		if (this->firstTime)
		{
			...
			dpb.insertPath(tags->plugin_list, this->pluginList);
		...
		dpb.insertBytes(tags->specific_data,
...
//------------------------------------------------------------------------
static inline void makeUtfString(bool const        uft8Convert,
                                 Firebird::string& s)
{
	if (uft8Convert)
	{
...
//------------------------------------------------------------------------
void ClntAuthBlock::extractDataFromPluginTo(CSTRING* const to)
{
...
//------------------------------------------------------------------------
void ClntAuthBlock::extractDataFromPluginTo(P_AUTH_CONT* const to)
{
...
//------------------------------------------------------------------------
const unsigned char* ClntAuthBlock::getData(unsigned int* const length)
{
...
//------------------------------------------------------------------------
void ClntAuthBlock::putData(CheckStatusWrapper* const status,
...
//------------------------------------------------------------------------
Firebird::ICryptKey* ClntAuthBlock::newKey(CheckStatusWrapper* const status)
{
	status->init();
